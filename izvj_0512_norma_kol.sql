...
set @datum1='2017-12-04 22:00:00'
set @datum2='2017-12-05 06:00:00';
    ...
    WITH MyTmpTable1 AS(
      SELECT NarZ.ID_Par,ens.linija, ENZ.Datum, (CASE Proizvodi.ZavrsnaObrada WHEN 'Tokarenje' THEN ENS.ObradaA WHEN 'Glodanje' THEN ENS.ObradaB WHEN 'Bušenje' THEN ENS.ObradaC ELSE ENS.ObradaA END) AS ZavrsnaObrada, 
        Proizvodi.ID_Pro, Proizvodi.VrstaPro, CAST(ISNULL(ens.Norma, 0) AS int) AS KolicinaOK, ENS.BrojRN
        ...
        WHERE ENZ.Datum BETWEEN @Datum1 AND @Datum2
            AND NarS.BazniRN = 0 AND NarS.Obrada1 = 0 ),
    MyTmpTable2 AS(
    ...
    SELECT MttPar.ID_Par, Partneri.NazivPar AS Kupac, MttPar.VrstaPro, 
      ...
      CAST(ISNULL((SELECT SUM(MttChi1.Kolicina) FROM MyTmpTable2 MttChi1 WHERE MttChi1.ID_Par = MttPar.ID_Par AND MttChi1.VrstaPro = MttPar.VrstaPro and mttchi1.linija not in ('HURCO','VC510','GT600-1','GT600-2','GT600-3') AND MttChi1.Vrsta = 'Norme'), 0) AS int) AS Proizvedeno_tok, 
--	  CAST(ISNULL((SELECT SUM(MttChi1v.cijena) FROM MyTmpTable2 MttChi1v WHERE MttChi1v.ID_Par = MttPar.ID_Par AND MttChi1v.VrstaPro = MttPar.VrstaPro and mttchi1v.linija not in ('HURCO','VC510','GT600-1','GT600-2','GT600-3') AND MttChi1v.Vrsta = 'Norme'), 0) AS int) AS Proizvedeno_vrij_tok,
	  CAST(ISNULL((SELECT SUM(MttChi12.Kolicina) FROM MyTmpTable2 MttChi12 WHERE MttChi12.ID_Par = MttPar.ID_Par AND MttChi12.VrstaPro = MttPar.VrstaPro and mttchi12.linija in ('HURCO','VC510','GT600-1','GT600-2','GT600-3') AND MttChi12.Vrsta = 'Norme'), 0) AS int) AS Proizvedeno_do, 
