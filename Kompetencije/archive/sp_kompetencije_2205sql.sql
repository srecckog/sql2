USE [RFIND]
GO
/****** Object:  StoredProcedure [dbo].[sp_Kompetencije_]    Script Date: 22.05.2017. 8:48:14 ******/
SET ANSI_NULLS ON
GO
...
FROM
  ...
  select radnikid , (stimulacija2/8) broj
  from [fxsap].dbo.plansatirada p
  where dbo.razlikamjeseci2( p.mjesec, p.godina ,12 )=1
  and stimulacija2>0
...
FROM
  ...
  select radnikid , sum((stimulacija2/8)) broj
  from [fxsap].dbo.plansatirada p
  where dbo.razlikamjeseci2( p.mjesec, p.godina ,6 )=1
  and stimulacija2 > 0
  group by radnikid
...
FROM
 ...
 select radnikid , sum(stimulacija2/8.0) broj
 from [fxsap].dbo.plansatirada p
 where dbo.razlikamjeseci2( p.mjesec, p.godina , 3)=1
 and stimulacija2>0 
 group by radnikid
...
FROM
  ...
  select radnikid , (stimulacija2/8) broj
  from [fxsap].dbo.plansatirada p
  where dbo.razlikamjeseci2( p.mjesec, p.godina ,1 )=1
  and stimulacija2>0
...
---------------------------------------------
   UPDATE  kompetencije set Stimulacija1=0 , Satnica=0
...
FROM
	...
	select p.radnikid id,sum(p.stimulacija1) stimulacija,p.satnicakn
	from [fxsap].dbo.plansatirada p
    where 
